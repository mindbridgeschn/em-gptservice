...
def process_request(task: dict):
    ...
    logger.info(f"[MINER-PROCESS-START] patient={patient_id} url={OCR_ENGINE_URL}")
    logger.debug(f"[MINER-TASK-PAYLOAD] patient={patient_id} payload={json.dumps(task, indent=2)}")
    ...
    logger.debug(f"[MINER-OCR-RESPONSE] patient={patient_id} response={json.dumps(ocr_response, indent=2)}")
    ocr_response= {
    ...
    if is_file:
        ...
        logger.debug(f"[MINER-DEMO-PAYLOAD] patient={patient_id} payload={json.dumps(backend_payload, indent=2)}")
    else:
        ...
        logger.debug(f"[MINER-EM-PAYLOAD] patient={patient_id} payload={json.dumps(backend_payload, indent=2)}")
        ...
        logger.info(f"[MINER-EM-TEXT] patient={patient_id} text_length={len(text_content)}")
    ...
    logger.info(f"[MINER-STORE] patient={patient_id} Result saved to Redis")
